...
def create_user(user: schemas.UserCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    hashed_resposta = auth.get_password_hash(user.resposta_secreta) if user.resposta_secreta else None
...
def update_user(user_id: int, user_update: schemas.UserUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def request_password_reset(request: schemas.PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    user = db.query(User).filter(User.email == request.email, User.ativo == True).first()
    if not user:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    
    if not user.pergunta_secreta:
        raise HTTPException(status_code=400, detail="Usuário não possui pergunta secreta cadastrada")
...
def confirm_password_reset(reset: schemas.PasswordReset, db: Session = Depends(get_db)):
    ...
    if not user.resposta_secreta:
        raise HTTPException(status_code=400, detail="Usuário não possui resposta secreta cadastrada")
...
def create_area(area: schemas.AreaCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.perfil != "admin":
...
def delete_area(area_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.perfil != "admin":
        raise HTTPException(status_code=403, detail="Apenas administradores podem excluir áreas")
...
def create_employee(employee: schemas.EmployeeCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if current_user.perfil == "lider":
        ...
        if employee.area not in user_areas:
            raise HTTPException(status_code=403, detail=f"Você só pode cadastrar funcionários das áreas: {current_user.area}")
...
def update_employee(employee_id: int, employee_update: schemas.EmployeeUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(Employee).filter(Employee.id == employee_id)
    ...
    if current_user.perfil == "lider" and employee_update.area:
        ...
        if employee_update.area not in user_areas:
            raise HTTPException(status_code=403, detail=f"Você só pode manter funcionários nas áreas: {current_user.area}")
    ...
    if employee_update.senha_assinatura:
        employee.senha_assinatura = auth.get_password_hash(employee_update.senha_assinatura)
...
def create_indicator(indicator: schemas.IndicatorCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def update_indicator(indicator_id: int, indicator_update: schemas.IndicatorUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def delete_indicator(indicator_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    indicator = db.query(Indicator).filter(Indicator.id == indicator_id).first()
    if not indicator:
        raise HTTPException(status_code=404, detail="Indicador não encontrado")
    db.delete(indicator)
...
def create_feedback(feedback: schemas.FeedbackCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def list_feedbacks(employee_id: int = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def sign_feedback(feedback_id: int, sign_data: schemas.FeedbackSign, db: Session = Depends(get_db)):
...
def delete_feedback(feedback_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if current_user.perfil != "admin":
        raise HTTPException(status_code=403, detail="Apenas administradores podem excluir feedbacks")
...
def download_feedback_pdf(feedback_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(Feedback).filter(Feedback.id == feedback_id)
    ...
    periodo = feedback.periodo_avaliado.replace(' ', '_').replace('/', '-') if feedback.periodo_avaliado else 'sem_periodo'
...
def get_dashboard_stats(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if current_user.perfil == "lider":
        ...
        feedbacks_query = feedbacks_query.join(Employee).filter(Employee.area.in_(user_areas))
    ...
    feedbacks_this_month_query = feedbacks_query.filter(Feedback.data_feedback >= current_month)
    
    employees_with_feedback = feedbacks_this_month_query.with_entities(Feedback.funcionario_id).distinct().all()
    ...
    return {
        ...
        "employees_without_feedback_this_month": [
            {"id": e.id, "nome": e.nome, "area": e.area} for e in employees_without_feedback
        ]
    }

# PERFORMANCE EVOLUTION ENDPOINT@app.get("/performance/evolution/{employee_id}")
def get_employee_evolution(employee_id: int, meses: int = 6, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    ).order_by(Feedback.data_feedback).all()
        ...
        try:
            ...
            for ind in indicadores:
                try:
                    ...
                    meta_str = str(ind.get('indicator_meta', '')).strip() if ind.get('indicator_meta') else None
...
def create_pdi(pdi: schemas.PDICreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if pdi.acoes:
        for acao in pdi.acoes:
            db_action = PDIAction(pdi_id=db_pdi.id, acao=acao.acao, descricao=acao.descricao)
            db.add(db_action)
        db.commit()
...
@app.get("/pdis")
def list_pdis(employee_id: Optional[int] = None, status: Optional[str] = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@app.get("/pdis/{pdi_id}")
def get_pdi(pdi_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@app.put("/pdis/{pdi_id}")
def update_pdi(pdi_id: int, pdi_update: schemas.PDIUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    pdi = db.query(PDI).filter(PDI.id == pdi_id).first()
    if not pdi:
        raise HTTPException(status_code=404, detail="PDI não encontrado")
...
def add_pdi_action(pdi_id: int, action: schemas.PDIActionCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    pdi = db.query(PDI).filter(PDI.id == pdi_id).first()
    if not pdi:
        raise HTTPException(status_code=404, detail="PDI não encontrado")
    
    db_action = PDIAction(pdi_id=pdi_id, acao=action.acao, descricao=action.descricao, concluida=action.concluida)
    db.add(db_action)
    db.commit()
...
def update_pdi_action(action_id: int, concluida: bool, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
