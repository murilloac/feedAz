...
def generate_feedback_pdf(feedback_data):
    ...
    if tipo_feedback == "PONTUAL":
        ...
        story.append(
            Paragraph(
                "Avaliação de Desempenho e Desenvolvimento Profissional", subtitle_style
            )
        )
    ...
    if tipo_feedback == "PONTUAL":
        ...
        if feedback_data.get("motivo_feedback"):
            ...
            motivo_box = Table(
                [[feedback_data["motivo_feedback"]]], colWidths=[17 * cm]
            )
            motivo_box.setStyle(
                TableStyle(
                    [
        ...
        if feedback_data.get("comentario_colaborador"):
            story.append(
                Paragraph("■ CONSIDERAÇÕES DO COLABORADOR", section_title_style)
            )
            story.append(
                Paragraph(feedback_data["comentario_colaborador"], content_style)
            )
            story.append(Spacer(1, 0.5 * cm))
    ...
    # FEEDBACK MENSAL - Formato completo
        ...
        if feedback_data.get("aderencia"):
            story.append(
                Paragraph(
                    "■ ADERÊNCIA A PROCESSOS E PROCEDIMENTOS", section_title_style
                )
            )
        ...
        if feedback_data.get("melhorias"):
            story.append(
                Paragraph("■ OPORTUNIDADES DE DESENVOLVIMENTO", section_title_style)
            )
    ...
    if feedback_data.get("assinado"):
        ...
        signature_data.append(
            ["Assinado por:", feedback_data.get("funcionario_nome", "N/A")]
        )
    else:
